...
macro_rules! frameworks {
    (
        ...
        $(
            struct $name;
            impl $crate::daemon::scanners::projects::framework_macro::FrameworkHeuristics for $name {
                fn name(&self) -> &'static str {
                    stringify!($name)
                }
                ...
                fn matches(&self, project_path: &str) -> bool {
                    ...
                    $(
                        $(
                            if !$crate::daemon::scanners::projects::framework_macro::has_file(project_path, $file) {
                                return false;
                            }
                        )*
                    )?
                    ...
                    $(
                        $(
                            if $crate::daemon::scanners::projects::framework_macro::has_directory(project_path, $dir) {
                                return true;
                            }
                        )*
                    )?
                    ...
                    $(
                        $(
                            if let Some(json) = $crate::daemon::scanners::projects::framework_macro::read_json_config(project_path, $json_file) {
                                if $crate::daemon::scanners::projects::framework_macro::check_json_path(&json, $json_path) {
                                    return true;
                                }
                            }
                        )*
        ...
        static HEURISTICS: &[&dyn $crate::daemon::scanners::projects::framework_macro::FrameworkHeuristics] = &[
            $(&$name {},)*
        ];
    };
}
